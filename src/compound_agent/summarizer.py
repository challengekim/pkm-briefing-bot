...
class Summarizer:
    ...
    def generate(self, prompt: str) -> str:
        """Generate LLM output from a fully formatted prompt string."""
        return self._generate(prompt)

    def generate_from_template(self, template_name: str, **kwargs) -> str:
        """Load a prompt template and generate LLM output.

        Public interface for template-based generation.
        """
        prompt = self._load_prompt(template_name).format(**kwargs)
        return self._generate(prompt)
